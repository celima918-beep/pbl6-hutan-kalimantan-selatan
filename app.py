...
if menu == "Halaman Utama & Identitas":
    ...
    fig_pie_profil = px.pie(df_melted, values="Luas (ha)", names="Fungsi Hutan", title="Proporsi Fungsi Kawasan Hutan Kalimantan Selatan")
    ...
    for idx, row in df_kabupaten.iterrows():
        if row["Total Kawasan Hutan (ha)"] > 0:
            popup_text = f"<b>{row['Kabupaten/Kota']}</b><br>Total Hutan: {row['Total Kawasan Hutan (ha)']:,.2f} ha"
            folium.Marker(location=[row["Latitude"], row["Longitude"]], popup=popup_text, icon=folium.Icon(color="green", icon="tree")).add_to(m)
    ...
    st.subheader("Proporsi Kontribusi Komponen Nilai terhadap Ekosistem Hutan")
    ...
    fig_pie_tev = px.pie(df_pie_tev, values="Nilai Ekonomi (Rp)", names="Komponen Nilai", title=f"Persentase Kontribusi Struktur TEV {pilihan_wilayah}")
